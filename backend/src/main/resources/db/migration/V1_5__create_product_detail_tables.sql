...
alter table cpus
    add constraint cpu_product foreign key (product_id) references products (id);
alter table cpus
    add constraint cpu_socket foreign key (socket_id) references sockets (id);

alter table motherboards
    add constraint motherboard_product foreign key (product_id) references products (id);
alter table motherboards
    add constraint motherboard_socket foreign key (socket_id) references sockets (id);
alter table motherboards
    add constraint motherboard_form_factor foreign key (form_factor_id) references motherboard_form_factors (id);
alter table motherboards
    add constraint motherboard_ram_memory_type foreign key (type_of_memory_id) references ram_memory_types (id);

alter table rams
    add constraint ram_product foreign key (product_id) references products (id);
alter table rams
    add constraint ram_memory_type foreign key (memory_type_id) references ram_memory_types (id);

alter table hard_disks
    add constraint hard_disks_product foreign key (product_id) references products (id);

alter table power_supplies
    add constraint power_supply_product foreign key (product_id) references products (id);
alter table power_supplies
    add constraint power_supply_category foreign key (power_supply_category_id) references power_supply_categories (id);

alter table computer_cases
    add constraint computer_case_product foreign key (product_id) references products (id);
alter table computer_cases
    add constraint computer_case_type foreign key (computer_case_type_id) references computer_case_types (id);
alter table computer_cases
    add constraint computer_case_motherboard_form_factor foreign key (max_motherboard_size_id) references motherboard_form_factors (id);

alter table cooling_systems
    add constraint cooling_system_product foreign key (product_id) references products (id);
alter table cooling_systems
    add constraint cooling_system_socket foreign key (socket_id) references sockets (id);

alter table video_cards
    add constraint video_card_product foreign key (product_id) references products (id);
alter table video_cards
    add constraint video_card_video_memory_type foreign key (video_memory_type_id) references video_memory_types (id);